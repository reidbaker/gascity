...
- **2026-05-23** — Isolation + port investigation COMPLETE (deferred from R3.1 flags). **Supervisor: SAFE** — each city runs in its own goroutine with an independent `recover()` in `cmd_supervisor.go:1827-1892`; a gctest crash cannot take down the supervisor or gc-management. No need to switch test city to standalone before R3.4. **Port clarification:** gc-management dolt = **28232** (confirmed, PID 3420191, `$GC_DOLT_PORT=28232`). Port **28231** = MCDClient rig (cwd `/home/jaword/projects/MCDClient/.beads/dolt`) — NOT gc-management. The bd memory "gascity bd on 28231" was stale; gascity rig dolt now runs on ephemeral ~42788. round3-test-city.md port table corrected. (architect)
- **2026-05-23** — Architect isolation+port investigation (gm-wisp-ia4t9): **SUPERVISOR SAFE** — per-city goroutines + panic recovery (cmd_supervisor.go:1827-1892); a test-city crash CANNOT affect gc-management. No standalone needed; supervisor-managed test city is safe even for R3.4 cut-over testing. **PORT MAP** (old "shared 28231" was stale — per-rig servers): gc-management/HQ dolt = **28232** (= the HQStore migration target), MCDClient = 28231, gascity ephemeral ~42788, test city = 28533. Corrected the MCD rig: 28232 is LIVE HQ (never kill); MCD fix = mcdclient on 28231. Verified 28232 alive + city-level bd healthy. (architect→mayor)
